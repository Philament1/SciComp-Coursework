...
Our initial value for the stochastic differential equations are \[
    ...
    \end{pmatrix}
\]
and we consider each solution for 1000 timesteps until $t=10$, so $\delta t = 0.01$. We consider cases forWe take the average of over 1000 simulations of the solution for each value of $mu$.

From the first figure, we observe that when $\mu = 0$, without the Brownian motion randomness, our solution quickly converges to 1 for all initial starting points. When $\mu = 0.2$, the solution appears to converge to a value below $1$, and when $\mu = 0.4$, the solution appears to converge to 0. This is due to the fact that increasing $\mu$ means the stochastic component of the SDE dominates the deterministic part more, hence the solutions fluctuate more around the average of the $0$ mean Normal distribution.

From the second figure, we observe that the greater the 
