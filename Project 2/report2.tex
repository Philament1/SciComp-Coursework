...
In the case where the source and target are disconnected or unreachable in the input graph, \textit{searchGPT} returns the value \textit{float(`inf')}, which can be interpreted as the minimum weight being infinite as there is no path. On the other hand, \textit{searchPKR2} returns a finite value along with an empty \textit{path} list. The value returned is the maximum solution to problem \eqref{eq:minimax} for all nodes in the subgraph that are connected to the source, hence the method is inapplicable to disconnected source and target nodes.
...
The function \textit{part2q3} attempts to solve the system of stochastic differential equations 
\[
    dY(t) = f(Y(t))dt + g(Y(T))dW(t)
\]where $f(Y(t))$ is the set of equations in \textit{part2q1} and $g(Y(t))=\mu$. The ``Brownian step'' of our system is denoted by $dW(t)$.
Our initial value for the stochastic differential equations is \[
    \mathbf{y_0} = 
    \begin{pmatrix}
        0.3 \\
        0.4 \\
    ...
    \end{pmatrix}
\]
The function solves this by applying the Euler-Maruyama method for 1000 timesteps until $t=10$, so $\Delta t = 0.01$. We consider cases for $\mu = 0, \pm 0.2, \pm 0.4, \pm 0.6, \pm 0.8, \pm 1$. We take the average of 1000 simulations of the solution for each value of $\mu$.
...
From the upper plot of Figure \ref{fig5}, we observe that when $\mu = 0$, without the Brownian motion component, our solution quickly converges to 1 for all initial starting points. When $\mu = 0.2$, the solution appears to converge to values below $1$, and when $\mu = 0.8$, the solution appears to converge to 0. It is clear that for larger values of $\mu$, there is more weight on the stochastic component of our system, hence there is more fluctuation in our solutions. 

From the lower plot of Figure \ref{fig5}, we observe that $\mu$ is symmetric for negative and positive values, which is expected as the Normal distribution is symmetric. We also observe that the greater the absolute value of $\mu$, the closer to 0 the solution is at $t=10$. This is due to the fact that increasing $\mu$ means the stochastic component of the SDE dominates the deterministic part more, hence the solutions fluctuate more around the average of the 0-mean Normal distribution.
