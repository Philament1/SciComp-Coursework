...
We observe from Figure \ref{fig_contour} that the solutions are oscillating initially for all $c$, and remain oscillating for $c=0.5$ and $c=1.2$, but the periodicity appears to break down gradually for $c=1.3$, and very quickly for $c=1.5$.
...
\begin{figure}[h!]
  ...
  \caption{Correlation dimension plots}
  \label{fig_corr}
\end{figure}

Figure \ref{fig_corr} shows the correlation dimension plots and the corresponding least squares fit in log-log scale. This fit is the correlation dimension. The dimension is less than 2 for $c=0.5$ and $c=1.2$, and more than 2 for $c=1.3$ and $c=1.5$, suggesting the system is stable when $c\leq1.2$.
